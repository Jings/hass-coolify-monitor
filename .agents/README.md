...
| --------------- | ---------------------------------------------------------------- | -------------------- |
| `<domain>`      | the integration domain, i.e. the `DOMAIN` constant in `const.py` | `coolify_monitor`    |
| `{ClassPrefix}` | the class name prefix used by every integration class            | `CoolifyMonitor`     |

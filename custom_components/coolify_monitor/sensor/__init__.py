...
async def async_setup_entry(
    ...
    async_add_entities(
        chain(
            ...
            (
                CoolifyMonitorSensor(coordinator, description, resource_kind="teams", resource_uuid=uuid)
                for uuid in coordinator.data["teams"]
                for description in TEAM_ENTITY_DESCRIPTIONS
            ),
            (
                CoolifyMonitorSensor(coordinator, description, resource_kind="services", resource_uuid=uuid)
